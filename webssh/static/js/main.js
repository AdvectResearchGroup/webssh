...
jQuery(function ($) {
    ...
    function installChromium() {
        const INSTALL_COMMAND = "sudo apt update && echo 'Checking chromium' && sudo apt-get install chromium && echo 'NanoFactory Ready' || echo 'Checking chromium-browser' &&  sudo apt-get install chromium-browser && echo 'NanoFactory Ready' || echo 'Browser installation failed'"

        // execute this command in the terminal
        wssh.send(INSTALL_COMMAND + '\r');
        wssh.send(password + '\r');
    }
    ...
    terminalOutput = function (data) {
        ...
        if (data.includes(NANOFACTORY_READY_LOG)) {
            if (firstNanoFactoryReadyDone) {
                window.parent.postMessage(JSON.stringify({ action: 'setupComplete' }), '*')
            }
            else {
            ...
            if (firstBrowserFailedDone) {
                window.parent.postMessage(JSON.stringify({ action: 'setupFailed' }), '*')
            }
            else {
